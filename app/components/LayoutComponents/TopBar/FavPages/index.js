...
class FavPages extends React.Component {
  ...
  componentDidMount() {
    const pagesList = () => {
      ...
      const localMenuData = JSON.parse(JSON.stringify(menuData));
      const flattenItems = (items, key) =>
        items.reduce((flattenedItems, item) => {
          ...
          if (Array.isArray(item[key])) {
            const itemList = item[key].map(child => {
              const childItem = child;
              childItem.icon = item.icon;
              return childItem;
            });
            return flattenedItems.concat(flattenItems(itemList, key));
          }
          flattenedItems.push(item);
          return flattenedItems;
        }, []);
      return flattenItems(localMenuData, 'children');
    };
    this.setState({
      pagesList: pagesList(),
    });
  ...
  generatePageList = searchText => {
    ...
    const localSearchText = searchText ? searchText.toUpperCase() : '';
    return pagesList.map(item => {
      const isActive = favs.some(child => child.url === item.url);
      if (
        !item.title.toUpperCase().includes(localSearchText) &&
        localSearchText
      ) {
        return null;
      }
